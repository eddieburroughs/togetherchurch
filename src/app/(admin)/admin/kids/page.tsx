...
import { listKids } from "@/features/kids/server/queries";
...
export default async function AdminKidsPage({ searchParams }: Props) {
  ...
  const { data: kids, count } = await listKids({ search, offset, limit });
    ...
    <main className="mx-auto max-w-4xl px-4 py-8">
          ...
          <div key={k.id} className="flex items-center justify-between px-4 py-3">
            <div>
              ...
              <p className="text-xs text-zinc-500">
                {k.dob && `DOB: ${k.dob}`}
                {k.allergies && ` · Allergies: ${k.allergies}`}
              </p>
            </div>
            {k.allergies && (
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between text-sm">
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link href={`/admin/kids?q=${search}&page=${page - 1}`}
